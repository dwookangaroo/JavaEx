...
public class ExceptionEx {
	...
	private static void throwExceptEx() {
		...
		try {
			...
			te.executeException(); // ioexception - checked - 반드시 예외처리필요
	...
	private static void arrayExceptionEx() {
		...
		try {
			...
			System.err.println("Error:" + e.getMessage());//Index 3 out of bounds for length 3
			e.printStackTrace(); //전체 아래정보 출력
		}
		System.out.println("end of code");
	}
	...
	private static void arithmeticExceptionEx() {
		...
		//예외가 없다: try - finally
		//catch 블럭 순서: 앞쪽에는 구체적인 예외 -> 마지막에 Exception으로(남아있을수있는 예외) 막는다
		try {
		// 예외 발생 가능 영역
		num = scanner.nextInt();
		result = 100 / num; 
		} catch (ArithmeticException e) {
			System.err.println("0읋는 나눌수없어요");
